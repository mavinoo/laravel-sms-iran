<?php
namespace Mavinoo\LaravelSmsIran\Gateways;

class FirstpayamakGateway extends GatewayAbstract {

	/**
	 * FirstpayamakGateway constructor.
	 */
	public function __construct() {

		$this->webService  = config('sms.gateway.firstpayamak.webService');
		$this->username    = config('sms.gateway.firstpayamak.username');
		$this->password    = config('sms.gateway.firstpayamak.password');
		$this->from        = config('sms.gateway.firstpayamak.from');
	}


	/**
	 * @param array $numbers
	 * @param       $text
	 * @param bool  $isflash
	 *
	 * @return mixed
	 * @internal param $to | array
	 */
	public function sendSMS( array $numbers, $text, $isflash = false ) {
		// Check credit for the gateway
		if(!$this->GetCredit()) return;

		$msg    = urlencode($text);
		$result = null;
		foreach($numbers as $number) {
			$result = file_get_contents("http://ui.firstpayamak.ir/tools/urlservice/send/?username={$this->username}&password={$this->password}&from={$this->from}&to={$number}&message={$msg}");
		}

		if($result) {
			return $result;
		}
	}


	/**
	 * @return mixed
	 */
	public function getCredit() {
		if(!$this->username and !$this->password)
			return 'Blank Username && Password';
		try {
			$client = new \SoapClient( $this->webService );

			$result = $client->GetCredit( array('username' => $this->username, 'password' => $this->password) );
			return $result->GetCreditResult;
		} catch( SoapFault $ex ) {
			echo $ex->faultstring;
		}
	}

}